...
订单还保存 `idempotency_key`。同一个用户不能使用同一幂等键创建两笔订单；购物车项增加版本字段，用乐观锁防止并发修改被静默覆盖。

## 接口

- `POST /api/orders`：从已选购物车项创建订单；无请求体，必须携带 `Idempotency-Key` 请求头。
...
1. 从 JWT 解析并校验当前用户。
2. 校验 `Idempotency-Key`；同一用户已使用该键时，直接返回原订单。
3. 查询该用户所有 `selected = true` 的购物车项。
4. 没有已选商品时，返回 `400 No selected cart items`。
5. 创建订单，并将每个购物车项复制为订单明细快照。
6. 基于购物车快照计算每项小计和订单总额。
7. 仅删除本次进入订单的购物车项。
...
- 缺少或无效的幂等键：`400 Idempotency key is required`。
- 乐观锁冲突：`409 Cart changed, please retry`。
