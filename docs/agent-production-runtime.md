...
部署前由一次性 migration Job 执行 `python scripts/setup_agent_checkpoint.py` 和 `python scripts/setup_agent_memory.py`；运行时不自动建表。LangGraph 官方将 SQLite 定位为本地/实验场景，并提供 `PostgresSaver` 作为生产数据库 checkpointer。[LangGraph persistence documentation](https://docs.langchain.com/oss/python/langgraph/persistence)
