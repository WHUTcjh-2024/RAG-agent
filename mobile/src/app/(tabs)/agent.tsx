...
const verdictLabels: Record<string, string> = { RECOMMEND_BUY: "建议购买", BUY_WITH_CAUTION: "谨慎购买", NOT_RECOMMENDED: "不建议购买", INSUFFICIENT_DATA: "信息不足" };
const verificationLabels: Record<string, string> = { PASSED: "全部校验通过", BLOCKED: "缺少可信事实", REJECTED: "不满足购买约束", PASS: "通过", FAIL: "未通过", SKIPPED: "未校验" };
...
export default function AgentScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      {pane === "evidence" && <ScrollView contentContainerStyle={styles.panel}>
        <Text style={styles.panelIndex}>03 / EVIDENCE</Text>
        <Text style={styles.panelTitle}>约束与推荐依据</Text>
        <View style={styles.slotCloud}>{Object.entries(store.slots).map(([key, item]) => <View key={key} style={styles.slot}><Text style={styles.slotKey}>{key.toUpperCase()}</Text><Text style={styles.slotValue}>{Array.isArray(item) ? item.join(" · ") : String(item)}</Text></View>)}</View>
        {store.agentProducts.length > 0 && <View style={styles.candidates}><Text style={styles.blockTitle}>候选商品 {store.agentProducts.length}</Text>{store.agentProducts.slice(0, 4).map((product) => <Pressable key={product.article_id} onPress={() => router.push(`/product/${product.article_id}`)} style={styles.candidate}><Image source={{ uri: productImage(product) }} style={styles.candidateImage} /><View style={styles.candidateCopy}><Text style={styles.candidateTitle} numberOfLines={1}>{product.prod_name}</Text><Text style={styles.candidateBody} numberOfLines={2}>{product.reason || `${product.product_type_name} · ${product.colour_group_name}`}</Text></View><ArrowRight size={14} color={colors.muted} /></Pressable>)}</View>}
        {store.decision?.fact_passport && <View style={[styles.passport, store.decision.fact_passport.status === "VERIFIED" ? styles.passportVerified : styles.passportIncomplete]}><View style={styles.passportHeader}><View style={styles.passportTitle}><Database size={13} color={colors.primary} /><Text style={styles.passportTitleText}>商品事实护照</Text></View><Text style={styles.passportStatus}>{store.decision.fact_passport.status === "VERIFIED" ? "事实齐全" : "事实待补充"}</Text></View><Text style={styles.passportMeta}>SKU {store.decision.fact_passport.sku_id || "未提供"} · {store.decision.fact_passport.version || "未提供版本"}</Text>{store.decision.fact_passport.facts.map((item) => <View key={`${item.source_id}-${item.field}`} style={styles.passportFact}><Text style={styles.passportFactName}>{item.field}</Text><Text style={styles.passportFactValue}>{item.value}</Text><Text style={styles.passportFactMeta}>{item.source_label || item.source_kind || item.source_type} · {item.verified ? "已核验" : "待核验"} · {Math.round((item.confidence || 0) * 100)}%</Text></View>)}{store.decision.fact_passport.missing_fields.length > 0 && <Text style={styles.passportMissing}>待补充：{store.decision.fact_passport.missing_fields.join("、")}</Text>}</View>}
        {(latestAnswer || store.evidence.length > 0) && <View style={styles.evidenceBlock}><Text style={styles.blockTitle}>推荐依据</Text>{latestAnswer && <View style={styles.linkedConclusion}><Text style={styles.conclusionLabel}>关联结论</Text><Text style={styles.conclusionText}>{latestAnswer.content}</Text></View>}{store.evidence.map((item, index) => <View key={`${item.source_id}-${item.field}-${index}`} style={styles.evidenceItem}><View style={styles.evidenceMeta}><Text style={styles.evidenceField}>{item.field}</Text><Text style={styles.evidenceType}>{item.source_label || item.source_type}</Text></View><Text style={styles.evidenceValue}>{item.value}</Text><Text style={styles.evidenceSource}>{item.source_id} · {item.verified ? "已核验" : "待核验"}</Text></View>)}</View>}
        {store.decision && <View style={styles.decision}><Text style={styles.decisionVerdict}>{verdictLabels[store.decision.verdict]}</Text><Text style={styles.decisionConfidence}>{Math.round(store.decision.confidence * 100)}%</Text>{store.decision.recommended_size && <Text style={styles.decisionSize}>建议尺码 {store.decision.recommended_size}</Text>}{store.decision.reasons.map((reason) => <Text key={reason} style={styles.decisionReason}>{reason}</Text>)}{store.decision.verification && <View style={styles.verification}><View style={styles.verificationHeading}><ShieldCheck size={13} color={colors.primary} /><Text style={styles.verificationTitle}>{verificationLabels[store.decision.verification.status]}</Text></View>{store.decision.verification.checks.map((check) => <View key={check.code} style={styles.verificationCheck}><Text style={[styles.verificationState, check.status === "FAIL" && styles.verificationFail]}>{verificationLabels[check.status]}</Text><View style={styles.verificationCopy}><Text style={styles.verificationLabel}>{check.label}</Text><Text style={styles.verificationMessage}>{check.message}</Text></View></View>)}</View>}{store.decision.missing_fields.length > 0 && <Text style={styles.passportMissing}>无法确认：{store.decision.missing_fields.join("、")}</Text>}</View>}
        {store.pendingAction && <View style={styles.confirm}><Text>等待你的确认</Text><Text>{store.pendingAction.summary}</Text><Pressable onPress={() => void confirm()}><Text>确认执行</Text></Pressable></View>}
      </ScrollView>}
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  slotCloud: { marginTop: 15, flexDirection: "row", flexWrap: "wrap", gap: 6 }, slot: { minWidth: 76, paddingHorizontal: 10, paddingVertical: 8, borderWidth: 1, borderColor: colors.line, borderRadius: radius.md, backgroundColor: colors.surface }, slotKey: { color: colors.muted, fontFamily: type.mono, fontSize: 6, fontWeight: "800", letterSpacing: 0.7 }, slotValue: { marginTop: 3, color: colors.ink, fontSize: 10, fontWeight: "700" }, candidates: { marginTop: 20 }, blockTitle: { marginBottom: 10, color: colors.ink, fontSize: 13, fontWeight: "800" }, candidate: { marginBottom: 6, padding: 6, flexDirection: "row", gap: 10, alignItems: "center", borderRadius: radius.md, backgroundColor: colors.surface }, candidateImage: { width: 54, height: 61, borderRadius: radius.sm }, candidateCopy: { flex: 1 }, candidateTitle: { color: colors.ink, fontSize: 11, fontWeight: "800" }, candidateBody: { marginTop: 3, color: colors.muted, fontSize: 9, lineHeight: 13 }, evidenceBlock: { marginTop: 20 }, linkedConclusion: { marginBottom: 8, padding: 12, borderWidth: 1, borderColor: "#DED3E4", borderRadius: radius.md, backgroundColor: colors.primaryPale }, conclusionLabel: { color: colors.primary, fontFamily: type.mono, fontSize: 7, fontWeight: "800", letterSpacing: 0.8 }, conclusionText: { marginTop: 7, color: colors.ink, fontSize: 11, lineHeight: 18 }, evidenceItem: { paddingVertical: 10, borderTopWidth: 1, borderTopColor: colors.line }, evidenceMeta: { flexDirection: "row", justifyContent: "space-between" }, evidenceField: { color: colors.ink, fontSize: 10, fontWeight: "800" }, evidenceType: { color: colors.primary, fontFamily: type.mono, fontSize: 7 }, evidenceValue: { marginTop: 5, color: colors.ink, fontSize: 10, lineHeight: 15 }, evidenceSource: { marginTop: 3, color: colors.muted, fontFamily: type.mono, fontSize: 7 }, passport: { marginTop: 18, padding: 12, borderWidth: 1, borderColor: colors.line, borderRadius: radius.md, backgroundColor: colors.surface }, passportVerified: { borderColor: "#ABD8C6", backgroundColor: "#F4FBF8" }, passportIncomplete: { borderColor: "#EAD5A1", backgroundColor: "#FFFAF0" }, passportHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 }, passportTitle: { flexDirection: "row", alignItems: "center", gap: 6 }, passportTitleText: { color: colors.ink, fontSize: 12, fontWeight: "800" }, passportStatus: { color: colors.primary, fontFamily: type.mono, fontSize: 7, fontWeight: "800" }, passportMeta: { marginTop: 7, color: colors.muted, fontFamily: type.mono, fontSize: 7 }, passportFact: { paddingTop: 8, marginTop: 8, borderTopWidth: 1, borderTopColor: colors.line }, passportFactName: { color: colors.ink, fontSize: 9, fontWeight: "800" }, passportFactValue: { marginTop: 3, color: colors.ink, fontSize: 10 }, passportFactMeta: { marginTop: 3, color: colors.muted, fontSize: 8 }, passportMissing: { marginTop: 8, color: "#A66B16", fontSize: 9, lineHeight: 14 }, decision: { marginTop: 13, padding: 13, borderRadius: radius.md, backgroundColor: colors.successSoft }, decisionVerdict: { color: colors.ink, fontSize: 12, fontWeight: "800" }, decisionConfidence: { marginTop: 3, color: colors.primary, fontFamily: type.mono, fontSize: 18, fontWeight: "800" }, decisionSize: { marginTop: 5, color: colors.ink, fontSize: 10, fontWeight: "700" }, decisionReason: { marginTop: 5, color: colors.muted, fontSize: 10, lineHeight: 15 }, verification: { marginTop: 10, paddingTop: 8, borderTopWidth: 1, borderTopColor: colors.line }, verificationHeading: { flexDirection: "row", gap: 6, alignItems: "center" }, verificationTitle: { color: colors.ink, fontSize: 10, fontWeight: "800" }, verificationCheck: { marginTop: 8, flexDirection: "row", gap: 7 }, verificationState: { width: 38, paddingTop: 1, color: colors.success, fontFamily: type.mono, fontSize: 7, fontWeight: "800" }, verificationFail: { color: "#A66B16" }, verificationCopy: { flex: 1 }, verificationLabel: { color: colors.ink, fontSize: 9, fontWeight: "800" }, verificationMessage: { marginTop: 2, color: colors.muted, fontSize: 8, lineHeight: 12 }, confirm: { marginTop: 13, padding: 14, borderRadius: radius.lg, backgroundColor: colors.surface },
});
